...
class AmSerialChartColumn():
    def __init__(self, category_field_value, graph_value_field_vlaue, graph_color_field_value='#8A0CCF'):
...
class AmSerialchartModel():
    def __init__(self):
        self.amchart_column_list = []
    
    def add_armchart_column(self, amchart_column, index=None):
        if index is not None:
            self.amchart_column_list.insert(index, amchart_column)
        else:
            self.amchart_column_list.append(amchart_column)
    
    def generate_armchart_column_json(self):
        return json.dumps(self.amchart_column_list)
    
    def get_am_chart_defination(self):
        am_chart_defination = {'serial_chart_category_field_tag':serial_chart_category_field_tag,
                               'serial_chart_graph_value_field_tag':serial_chart_graph_value_field_tag,
                               'serial_chart_graph_color_field_tag': serial_chart_graph_color_field_tag,
...
class VEXAmSerialchartModel(AmSerialchartModel):
    ...
    def _sort(self):
        ...
        armchart_column_keys = ['#FF0F00', '#FF6600', '#FF9E01', '#FCD202', '#F8FF01', '#B0DE09', '#04D215', '#0D8ECF', '#0D52D1', '#2A0CD0', '#8A0CCF']
        ...
        time_distribute_key.sort()
        
        tmp_list = []
        for i, key in enumerate(time_distribute_key):
            armchart_column = armchart_column_dict.get(key)
            if armchart_column.get(serial_chart_graph_color_field_tag) =='':
                armchart_column[serial_chart_graph_color_field_tag] = armchart_column_keys[i%len(armchart_column_keys)]
            tmp_list.append(armchart_column)
        self.amchart_column_list = tmp_list
    
    def generate_armchart_column_json(self):
        self._sort()
        super.generate_armchart_column_json()     

def generate_vex_am_serial_chart_json(time_distribution_list):
    ...
    return vex_chart.generate_armchart_column_json()  
